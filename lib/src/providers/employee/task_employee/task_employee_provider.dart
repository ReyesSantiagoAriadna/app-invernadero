...
class TaskEmployeeProvider{
  static final TaskEmployeeProvider _TaskEmployeeProvider = TaskEmployeeProvider._internal();

  factory TaskEmployeeProvider() {
    return _TaskEmployeeProvider;
  }

  TaskEmployeeProvider._internal();
  ...
  Future<Map<DateTime, List<TareasTrabajadorElement>>> loadMyTasks(int idPersonal)async{  
    if(_loading)return null;
    _loading=true;
      
    _page++;
    int pag=1;
    final url = "${AppConfig.base_url}/api/personal/tareas_trabajador?page=$pag&personal=$idPersonal"; 
    ...
    final response = await http.get(
      url, 
      headers: headers,);
    print("RESPUESTA DE TAREAS TRABAJADOR");
    print(response.body);
    ...
    if(response.body.contains("tareas_trabajador") && response.body.contains("id_tarea")){
      TareasTrabajador tareaTrabajador = TareasTrabajador.fromJson(json.decode(response.body));
      _loading=false;
      return tareaTrabajador.tareasTrabajador;
    }
    _loading=false;
    return null;
  }   
  ...
  Future<Map<String,dynamic>> iniciarTarea(int consecutivo)async{  
      print("consecutivo $consecutivo");
      final url = "${AppConfig.base_url}/api/personal/iniciar_tarea"; 
      final token = await _storage.read('token');
      Map<String, String> headers = {
        HttpHeaders.authorizationHeader: "Bearer $token",
        "Accept": "application/json",};
      final response = await http.put(
        ...
        ;
      print("RESPUESTA AL INICIAR TAREA PERSONAL");
      print(response.body);
  ...
  Future<Map<String,dynamic>> solicitarFinalizacion(int consecutivo)async{  
      print("consecutivo $consecutivo");
      final url = "${AppConfig.base_url}/api/personal/solicitar_finalizacion"; 
      final token = await _storage.read('token');
      Map<String, String> headers = {
        HttpHeaders.authorizationHeader: "Bearer $token",
        "Accept": "application/json",};
...
//     if(response.body.contains("fecha") && response.body.contains("id_tarea")){
//       TareasPersonal tareasPersonal = TareasPersonal.fromJson(json.decode(response.body)); 
...
//     if(response.body.contains("fecha") && response.body.contains("id_tarea")){
//       TareasPersonal tareasPersonal = TareasPersonal.fromJson(json.decode(response.body)); 
//        return {'ok':true, 'tarea_personal' : tareasPersonal ,'message' : "Tarea modificada"};
//     }
//     return {'ok':false, 'tarea_personal' : null ,'message' : json.decode(response.body)['message']};
//   } 
//   //reasignar_horario
