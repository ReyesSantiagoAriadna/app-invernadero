...
class TareaEditPage extends StatefulWidget {
  TareaEditPage({Key key}) : super(key: key);

  @override
  _TareaEditPageState createState() => _TareaEditPageState();
}
...
class _TareaEditPageState extends State<TareaEditPage> {
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
     if(tarea==null){
      ...
      print("TAREA=>>> HORAS ${tarea.horaInicio} ${tarea.horaFinal}");
      genericBloc.onChangeNombre(tarea.nombre);
      genericBloc.onChangeHoraIn(tarea.horaInicio);
      genericBloc.onChangeHoraFin(tarea.horaFinal);
      genericBloc.onChangeDescripcion(tarea.detalle);
      tareaBloc.onChangetTareTipo(tarea.tipo);
      tareaBloc.init(context,tarea);
      ...
      children: <Widget>[
        Positioned(
          ...
          appBar: AppBar(
            ...
            backgroundColor: Colors.white,
            title:Text("Editar Tarea",style:TextStyle(color: MyColors.GreyIcon,
  ...
  _inputNombre(){
    return StreamBuilder(
        ...
        initialData: genericBloc.nombre,
        ...
        children:<Widget>[
          ...
          Container(
            ...
            child: StreamBuilder<Object>(
              ...
              initialData: genericBloc.descripcion,
              builder: (context, snapshot) {
                return TextFormField(
                  initialValue: snapshot.data,
                  maxLines: 4,
  ...
  _times(){
    return Row(
      children: <Widget>[
        _time("Hora de inicio", 1,genericBloc.horaIniStream,genericBloc.onChangeHoraIn,genericBloc.horaIni),
        Expanded(child: Container()),
        _time("Hora de Finalización", 2,genericBloc.horaFinStream,genericBloc.onChangeHoraFin,genericBloc.horaFin),
      ],
    );
  }
  
  _time(String title,int tipo,Stream stream,Function(String) onChange,String data){
    return GestureDetector(
      onTap: (){
        if(tipo==1)
              ...
              Expanded(
                  ...
                  child: StreamBuilder(
                    ...
                    builder: (BuildContext context, AsyncSnapshot snapshot){
                      if(!snapshot.hasData)
                        return Container();
                      List<h.Herramienta> herramientas= snapshot.data;
  ...
  _itemHerramienta(h.Herramienta h){
    return new ListTile(
                ...
                child: Container(
                  ...
                  child: StreamBuilder(
                    ...
                    builder: (BuildContext context, AsyncSnapshot snapshot){
                      if(!snapshot.hasData)
                        return Container();
                      List<i.Insumo> insumos= snapshot.data;
                      return ListView.builder(
                        physics: BouncingScrollPhysics(),
  ...
  _createButtonSave(){
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot){
          ...
          onPressed: snapshot.hasData?()=>_updateTarea():null
          );
      },
    );
  }
  ...
  _updateTarea()async{
    ...
    await tareaBloc.updateTarea(context,tarea.id);
    setState(() {
      _isLoading=true;
    });     
