...
class SearchSolares extends SearchDelegate{
  final solaresProvider = new SolaresCultivosProvider();
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    return FutureBuilder(
      future: solaresProvider.filterSolares(query),
      builder: (BuildContext context,AsyncSnapshot<List<Solar>> snapshot){
        if(snapshot.hasData){
              ...
              children: productos.map((p){
                return ListTile(
                  leading: Icon(LineIcons.sun_o),
                    title: Text("# ${p.id} ${p.region}"),
                    subtitle: Text("${p.municipio}"),
                    onTap: (){
                      close(context, null);
